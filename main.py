...
def main():
    ...
    os.environ['SDL_VIDEO_WINDOW_POS'] = '700,100' #НАЧАЛЬНЫЕ КООРДИНАТЫ ОКНА
    ...
    while running:
        ...
        dist_list.append(sat.distance)
            ...
            #получать данные с клавы

        for event in pg.event.get():
            if event.type == pg.QUIT:
                running = False
            elif event.type == pg.KEYDOWN and event.key == pg.K_ESCAPE:
                screen = pg.display.set_mode((800,645)) # выход полн. экран. режима
            elif event.type == pg.KEYUP:
                sat.thrust.stop()
                mapping_off(planet)        

            elif mapping_enabled:
                if event.type == pg.KEYDOWN and event.key == pg.F_m:
                    mapping_on(planet)
    ...
    if tick_count % (eccentricity_calc_interval * fps) == 0:
        eccentricity = eccentricity_calc_interval(dist_list)
        dist_list = []
